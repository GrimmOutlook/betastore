class PasswordResetsController < ApplicationController
	...
	def create
	  ...
	  if customer
		...
		redirect_to '/', notice: 'Check your email for instructions on resetting your password'
	  else
		render 'new'
	  end
    end
    ...
    def update
    	if @password_reset.customer.update_attributes(password: params[:password])
    	  ...
    	  logger.info @password_reset.customer.errors.full_messages
    	  render 'edit'  
    	end
    end
    ...
    def check_token
      ...
      unless @password_reset.try(:token) == params[:token]
      	redirect_to forgot_password_path, alert: 'Your password reset link has expired, please enter your
      	 email address again to get an email with a new link'
      end
    end
